...
MStatus  	LaplacianMeshDeformer::deform(MDataBlock& 	_data, MItGeometry& 	_iter,   const MMatrix& 	_mat,  unsigned int		_multiIndex)
{
    ...
    if(m_laplaceMatrix.nonZeros()==0)
	{
        ...
        bool cotWeights = weightTypeHandle.asBool();
        if(cotWeights) MGlobal::displayInfo("Cotangent Weights Selected");
        else MGlobal::displayInfo("Umbrella Weights Selected");
...

{
    ...
    m_cotWeights = handlesAddedAttr.create("cotWeightEnbl","cw",MFnNumericData::kBoolean,true,&stat);
